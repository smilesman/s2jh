...
public abstract class PersistableController<T extends PersistableEntity<ID>, ID extends Serializable> extends
    ...
    protected Set<String> getParameterIds(String paramName) {
        ...
        String[] ids = this.getRequest().getParameterValues(paramName);
        if (ids != null && ids.length > 0) {
            idSet.addAll(Sets.newHashSet(ids));
        }
        String[] ids2 = this.getRequest().getParameterValues(paramName + "[]");
        if (ids2 != null && ids2.length > 0) {
            idSet.addAll(Sets.newHashSet(ids2));
        }
        return idSet;
    }
    ...
    protected Collection<T> getEntitiesByParameterIds() {
        ...
        for (String id : getParameterIds()) {
            T entity = getEntityService().findOne((ID) id);
            entities.add(entity);
        }
        return entities;
    }
    ...
    protected HttpHeaders doDelete() {
        ...
        getEntityService().delete(enableDeleteEntities);
    ...
    protected String convertPropertyDisplay(Object entity, Field field, Object value) {
        ...
        if (value instanceof PersistableEntity) {
            ...
            try {
                ...
                UtilService utilService = SpringContextHolder.getBean(UtilService.class);
                try {
                    ...
                    Object obj = utilService.findEntity(entityClass, id);
