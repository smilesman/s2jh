...
public class UtilController extends RestActionSupport implements ModelDriven<Object> {
    ...
    /** 缓存对象 */
    private static Map<Boolean, String> booleanCachedKeyValuesMap = null;

    @MetaData(title = "布尔类型Map数据")
    @SecurityControllIgnore
    public HttpHeaders booleanMapData() {
        if (booleanCachedKeyValuesMap == null) {
            booleanCachedKeyValuesMap = Maps.newHashMap();
            booleanCachedKeyValuesMap.put(Boolean.TRUE, "是");
            booleanCachedKeyValuesMap.put(Boolean.FALSE, "否");
        }
        model = booleanCachedKeyValuesMap;
