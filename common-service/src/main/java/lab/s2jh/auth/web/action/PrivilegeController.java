...
import lab.s2jh.core.service.BaseService.R2OperationEnum;
        ...
        for (PrivilegeUrlVO url : urls) {
            ...
            for (Privilege privilege : privileges) {
                ...
                if (StringUtils.isNotBlank(privilegeUrl)) {
                    if (!privilegeUrl.endsWith("**")) {
                        privilegeUrl = privilegeUrl + "**";
                    }
                    if (urlMatcher.match(privilegeUrl, url.getUrl())) {
                        url.getControllPrivileges().add(privilege.getUrl());
